...
40. **One 660-line `<script>` block with 13 module-level mutable globals** (`videos`, `listTitle`, `lists`, `activeId`, `loopPlaylist`, `editingId`, `editingTitle`, `draggedId`, `dropTargetId`, `dropPosition`, `jamPlayer`, `jamQueue`, `jamIndex`). Single-file is a hard constraint, but the code can still be grouped into clearly-marked sections (state / storage / render / drag / player / io) with section banners.
41. **No JSDoc on the storage shape.** The on-disk schema must be reconstructed by reading `loadState`. A single typedef at the top would document it in one place.
42. **Drag state isn't cleared on Escape.** If the user starts a drag and presses Esc, `draggedId` stays set until the next dragend.
